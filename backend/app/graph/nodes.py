...
# ── Enhanced retrieval functions (Task 1-7) ───────────────────────────────────
...
async def retrieve_vector_node(state: GraphState) -> dict:
    """Task 5: Vector retrieval with metadata filtering."""
    logger.info("[LangGraph] retrieve_vector_node")
    try:
        # Extract citations for metadata filtering
        citations, _ = citation_extractor.extract_from_question(state["question"])
        ctx, hits = await fetch_vector_context(state["question"], citations)
        return {
            "vector_context": ctx,
            "vector_hits": hits,
            "citations": citations,
